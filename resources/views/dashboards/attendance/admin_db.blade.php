    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {const popup = document.getElementById('popup-info');
            ...
            document.querySelectorAll('.user-card').forEach(card => {card.addEventListener('click', () => {const userId = card.dataset.userToken;
                ...
                fetch("{{ route('attendance.fetch') }}", {method: "POST",
                    ...
                    .then(data => {if (!data || (data.attendances.length === 0 && data.teacher.length === 0)) {popupContent.innerHTML = '<span class="text-xl text-gray-800 dark:text-gray-300 mb-6">No attendance found.</span>';} else {let attendances = data.attendances;
                        ...
                        let subjectsMap = {};
                        attendances.forEach(g => {if (!subjectsMap[g.name]) {subjectsMap[g.name] = [];}

                            const date =new Date(g.date.slice(0, 10))
                                                    ...
                                                    <div class="flex gap-2">
                                                        ...
                                                        <div class="">
                                                            <div
                                                                ...
                                                                onclick="showGradeDetails('${entry.status}', '${entry.comment}', '${entry.teacher}', '${entry.date}', '${entry.from}', '${entry.to}')"
                                                            >
                                                                ${entry.date}
                                                            </div>
                                                        </div>
        ...
        function formatHourFromInt(num) {
            const str = num.toString().padStart(4, '0');
            return str.slice(0, 2) + ':' + str.slice(2);
        }

        function showGradeDetails(status, comment, teacher, date, hfrom, hto) {const content = `
                    <div class="flex justify-between items-start ">
                        <span class="font-medium text-gray-500 dark:text-gray-300">Status:</span>
                        <span class="text-right text-gray-800 dark:text-gray-100">${status}</span>
                    </div>
                       <div class="flex justify-between items-start">
                        <span class="font-medium text-gray-500 dark:text-gray-300">Comment:</span>
                        <span class="text-right text-gray-800 dark:text-gray-100">${comment}</span>
                    </div>
                    <div class="flex justify-between items-start">
                    ...
                    <div class="flex justify-between items-start">
                        <span class="font-medium text-gray-500 dark:text-gray-300">Hour:</span>
                        <span class="text-right text-gray-800 dark:text-gray-100">
                            From: ${formatHourFromInt(hfrom)} To: ${formatHourFromInt(hto)}
                        </span>
                    </div>
            `;
